...
The contract does not download, execute, or authenticate referenced artifacts or commands. A SUPPORTED label means the declaration contains the required indexed evidence, not that the real-world claim is true. Applications must disclose these limits beside results and use a fresh deployment when the authoritative source collection or policy changes.
